 ...
 *
 * <p>
 * 背景（业务要求）：管理员创建一个账户后，该账号在各业务表里的档案应当<b>同时</b>建好， 而不是等到用户第一次访问功能时才懒创建——否则「账户存在但查不到自己的学籍/账户」会成为一个
 * 需要到处判空的常态。
 *
 * <p>
 * 但「建行」只对<b>一账号一行</b>的档案表成立（用户表、学籍表、图书馆账户表）； 借阅记录、选课记录、订单这类<b>行为记录表</b>是「一账号多行」，注册时预建空行属于脏数据，
 * 应当在首次行为发生时写入，因此对应的模块<b>不实现</b>本接口。
 *
 * <p>
 * 落库映射（{@code sql/vCampus.sql}）：
 *
 * <table border="1">
 ...
 * </tr>
 * <tr>
 * <td>{@code tblUserCredential}</td>
 * <td>账户本身</td>
 * <td>是（用户模块自己负责，见 {@code JdbcUserRepository}）</td>
 * </tr>
 * <tr>
 * <td>{@code tblStudentProfile}</td>
 * <td>1:1 学籍档案</td>
 ...
 * </tr>
 * <tr>
 * <td>{@code tblCourseStudent} / {@code tblTeacher}</td>
 * <td>1:1 选课侧档案</td>
 * <td>是（按角色二选一；管理员不建）</td>
 * </tr>
 * <tr>
 ...
 * </tr>
 * <tr>
 * <td>{@code tblBorrow} / {@code tblCourseSelection} / {@code tblOrder}</td>
 * <td>行为记录</td>
 ...
 *
 * <p>
 * 实现约定：{@link #provision} 必须<b>幂等</b>（重复调用不产生第二条档案）， 失败时抛运行时异常——装配层会回滚已建档案并撤销账户，不允许留下「半个账户」。
 */
