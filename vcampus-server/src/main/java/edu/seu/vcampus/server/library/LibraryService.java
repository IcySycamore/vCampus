...
public class LibraryService {
     ...
     */
    public BorrowRecord borrow(String userId, String isbn) throws SQLException, LibraryException {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            try {
                ...
                if (borrowDao.hasActive(connection, validUser, validIsbn)) {
                    throw new LibraryException(StatusCode.BAD_REQUEST, "不能重复借阅同一本书");
                }
                if (!bookDao.adjustAvailable(connection, validIsbn, -1)) {
                    throw new LibraryException(StatusCode.BAD_REQUEST, "该书暂无可借馆藏");
                }
                Date now = new Date();
                BorrowRecord record = new BorrowRecord(validUser, validIsbn, book.getTitle(), now,
                        dueDate(now));
    ...
    public BorrowRecord returnBook(String userId, long recordId)
        ...
        try (Connection connection = dataSource.getConnection()) {
            connection.setAutoCommit(false);
            try {
                BorrowRecord record = borrowDao.findActiveById(connection, validUser, recordId);
                if (record == null) {
