...
| -------- | ------- | ------------------------------- | --------------- | ------------------------------------------------------------------ |
| 用户管理 | 100–199 | `client.user.UserService`       | ✅ `UserModule` | 只有 100/101/102/110 可用；106/107/108 常量已定义但**未注册**      |
...
| 104     | `USER_UNREGISTER`       | ⛔               | `{ userName }`                              | —                               | 管理员            |
| 105     | `USER_BATCH_UNREGISTER` | ⛔               | `{ List<String> userNames }`                | —                               | 管理员            |
| 106     | `USER_LIST`             | ⛔（常量已定义） | `UserQuery`                                 | `PageResponse<User>`            | 管理员            |
| 107     | `USER_UPDATE`           | ⛔（常量已定义） | `UserUpdateRequest`                         | —                               | 管理员            |
| 108     | `USER_TOGGLE_ENABLED`   | ⛔（常量已定义） | `{ userName, enabled }`                     | —                               | 管理员            |
| **109** | `USER_CHANGE_PASSWORD`  | **新增**         | `ChangePasswordRequest`                     | —                               | 本人 / 管理员重置 |

> **109 为什么不用明文**：旧密码用挑战-应答校验（复用 100 + `NonceManager`，发送 `proof` 而非旧密码）；
> 新密码由**客户端生成新盐**，只提交 `newSalt` 与 `sha256(newSalt + 新密码)`，
> 服务端直接落库为新的 `H`，全程没有明文密码上线。管理员重置走同一命令，但校验的是管理员会话（需 `USER_MANAGE`）。

`User` 实体需新增 `enabled` 字段（否则 108 无落点）；`enabled=false` 时 100 回 `P102`（常量已存在）。

### 4.2 客户端 API：`client.user.UserService`

```java
// 会话
void login(String userName, Role role, String password);   // 成功后 write-through 到 ClientSession
void logout();
boolean isLoggedIn();
...
void batchRegister(List<RegisterRequest> requests);
void unregister(String userName);
void batchUnregister(List<String> userNames);
PageResponse<User> listUsers(UserQuery query);
void updateUser(UserUpdateRequest request);
void toggleUserEnabled(String userName, boolean enabled);
```

**相对现状的改动**：去掉 `throws IOException, InterruptedException`（异常统一为 `ApiException`）；
`String role` → `Role` 枚举；`getSession()` 不再对外（改 `currentSession()`），`ClientSession` 退回包内细节。
...
| 文件                               | 改动                                                                                                            |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `constant/Command.java`            | 加 `USER_CHANGE_PASSWORD = 109`、`USER_SEGMENT_START/END`                                                       |
| `user/AuthServiceHandler.java`     | 注册 103/104/105/106/107/108/109；`requireAdmin` → `requireCapability(request, sender, Capability.USER_MANAGE)` |
| `user/AuthService.java`            | 增 `changePassword / listUsers / updateUser / setEnabled / unregister / batchRegister`                          |
| `user/UserRepository.java`         | 增 `findAll(query) / count(query) / findByUuid / update / setEnabled / softDelete`                              |
| `user/InMemoryUserRepository.java` | 实现上述方法（含 enabled 过滤与分页）                                                                           |
| `user/AuthModule.java`             | 仍只返回 `SessionManager`；无需改签名                                                                           |
...
| #   | 缺口                                                                                 | 位置                                       | 归属            |
| --- | ------------------------------------------------------------------------------------ | ------------------------------------------ | --------------- |
| 1   | 106/107/108 未注册；109 不存在                                                       | `AuthServiceHandler` / `Command`           | 用户管理        |
| 2   | `UserRepository` 无分页查询/更新/启停                                                | `UserRepository` 及其实现                  | 用户管理        |
| 3   | `User` 无 `enabled` 字段                                                             | `common.user.entity.User`                  | 用户管理        |
