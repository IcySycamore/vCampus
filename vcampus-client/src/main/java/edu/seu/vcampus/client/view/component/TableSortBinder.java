 ...
 *
 * <p>
 * 与 {@link PageBarPanel} 一样只做「显示与意图上报」：本类不改数据也不发请求，排完把回调一跑， 由页面带着新的字段与方向重新查询。
 *
 * <p>
 * <b>为什么排序不在这里做</b>：列表是分页的，本地只能排「当前这一页」——页内有序、页间乱序， 翻到第二页就会发现开头的数字比第一页的结尾还大。所以真正的排序交给服务端，本类只产生
 * 「按第几列、什么方向」这个意图。
 *
 * <p>
 * 表头文字用 ▲ / ▼ 标出当前排序列与方向：没有这个记号，用户点完看不出到底排没排、朝哪边排。 列下标 → 字段的映射由调用方给出，映射返回 null
 * 的列（例如「审核意见」这类自由文本）点不动。
 *
...
public final class TableSortBinder<T> {
     ...
     *
     * <p>
     * 原始文字从表格模型取（{@code getColumnName}），而不是在第一处保留一份副本——模型里的列名 才是唯一真相，另存一份迟早在改列时会不同步。
     */
