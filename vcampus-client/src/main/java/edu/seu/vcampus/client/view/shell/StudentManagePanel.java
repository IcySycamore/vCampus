 ...
 * 单个按钮再各自判能力（教师因此只看得到列表、看不到任何写操作）；真正的准入在服务端，越权一律回 403。
 * <b>看详情不受此限</b>：它是纯只读的，教师和学生都不能改却能看，所以对谁都可用。
 */
public class StudentManagePanel extends JPanel {
    ...
    /**
     * 打开选中行的档案详情（纯只读，不区分角色）。
     */
    private void showDetail() {
        StudentProfile target = selected();
        if (target == null) {
            JOptionPane.showMessageDialog(this, "请先在表格里选中一行学籍", "提示",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }
        StudentProfileDetailDialog.open(this, target);
    }

    /** 底部：左侧「查看详情」与按能力显示的操作按钮，右侧分页栏。 */
