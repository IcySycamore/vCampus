...
class AdminConsolePanel extends RoundedPanel {
     ...
     * 构造管理控制台。
     *
     * @param userApi 用户管理 API
     * @param studentApi 学籍 API；未装配时可为 null（此时不挂学籍页签）
     * @param role 当前身份，决定学籍页签出不出现
     */
    AdminConsolePanel(UserService userApi, StudentService studentApi, Role role) {
        super(new BorderLayout(), 18, UiTheme.SURFACE);
        setBorder(BorderFactory.createEmptyBorder(16, 16, 16, 16));
        JTabbedPane tabs = new JTabbedPane();
        tabs.setUI(new ModernTabbedPaneUI());
        tabs.addTab("用户管理", new UserManagePanel(userApi));
