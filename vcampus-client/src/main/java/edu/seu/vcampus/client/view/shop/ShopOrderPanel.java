...
public class ShopOrderPanel extends JPanel {
    ...
    public ShopOrderPanel(ShopService api) {
        ...
        cancelButton = UiFactory.secondaryButton("取消订单", null);
        ...
        // 订单表格
        String[] columns = {"订单ID", "商品名称", "数量", "总价", "状态", "下单时间"};
    ...
    private void displayOrders(OrderListResponse response) {
        ...
        if (response.getOrders() != null) {
            for (ShopOrder order : response.getOrders()) {
                ...
                row[0] = order.getoId();
                row[1] = order.getoItemId(); // 显示商品ID，因为ShopOrder没有商品名称字段
                row[2] = order.getoQuantity();
                row[3] = "¥" + order.getoTotal(); // 使用总价
                row[4] = order.getoStatus().getDisplayName();
                row[5] = order.getoTime() != null ? dateFormat.format(order.getoTime()) : "";
    ...
    private void cancelSelectedOrder() {
        ...
        UiTasks.run(new UiTasks.Task<Void>() {
            ...
            @Override
            public void accept(ApiException error) {
                setBusy(false, "取消失败: " + error.getMessage());
                feedback.setForeground(UiTheme.ACCENT);
            }
        });
    }
    ...
    private void updateButtonStates() {
        ...
        payButton.setEnabled("未支付".equals(status));
        cancelButton.setEnabled("未支付".equals(status) || "已支付".equals(status));
    }
    ...
    private void setBusy(boolean value, String message) {
        ...
        payButton.setEnabled(!value && orderTable.getSelectedRow() >= 0);
        cancelButton.setEnabled(!value && orderTable.getSelectedRow() >= 0);
